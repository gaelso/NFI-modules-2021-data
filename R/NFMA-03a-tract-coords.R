...
check_coord <- newplot02 %>%
  select(iso, plot_long, plot_lat,  plot_x, plot_y, epsg, epsg_corr) %>%
  group_by(iso) %>%
...
newtract03tmp <- newplot02 %>%
  ...
  select(iso, tract_id, nb_plot, plot_no_min, plot_lat, plot_long, x_corr, y_corr, epsg,  epsg_corr)
...
conv_utm <- map_dfr(.x = vec_epsg, .f = function(x, .tract = tract_latlong){
  ...
  #   print(map1)
    ...
    sf_in %>%
      bind_cols(utm_x = st_coordinates(.)[,1], utm_y = st_coordinates(.)[,2]) %>%
      as_tibble() %>%
      select(iso, tract_id, utm_x, utm_y)
...
newtract03tmp2 <- newtract03tmp %>%
  ...
  select(!c(plot_lat, plot_long, x_corr, y_corr, utm_x, utm_y))
...
conv_latlong <- map_dfr(.x = vec_epsg, .f = function(x, .tract = newtract03tmp2){
  ...
  sf_in %>%
    bind_cols(tract_long = st_coordinates(.)[,1], tract_lat = st_coordinates(.)[,2]) %>%
    as_tibble() %>%
    select(iso, tract_id, tract_long, tract_lat)
