...
sf_tract$envir_stress <- terra::extract(envir_stress, sf_tract)$layer
...
if (nrow(tt) > 0) {
  ...
  tt$envir_stress <- terra::extract(envir_stress, tt, buffer = 20000, fun = mean)$layer
...
newtract03 <- newtract03tmp |>
  mutate(
    tract_long = st_coordinates(newtract03tmp)[,1],
    tract_lat  = st_coordinates(newtract03tmp)[,2]
  ) |>
  as_tibble() |>
  dplyr::select(
