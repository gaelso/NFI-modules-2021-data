...
create_newland <- function(.seed = 11, .alt = 2000, .sea = 0.2, .nb_ft = 4, .mg = T, .river = F, .path = "C:/SAGA-GIS", .tmp = T, .quiet = T){
  ...
  topo_mpd <- terra::rast(topo_mpd)
  topo_mpd <- terra::crop(topo_mpd, terra::ext(0, ll, 0, ll))
  # topo_mpd
  # plot(topo_mpd)
  
  ## --- Apply sea ratio and max altitude -----------------------------------
  ## --- Translate a percentage of the pixels equal to .sea below 0
  # topo_init <- topo_mpd - terra::quantile(topo_mpd, probs = .sea)
  topo_init <- topo_mpd - as.numeric(terra::global(topo_mpd, fun = quantile, probs = .sea))
  # plot(topo_init)
  
  ## --- Apply different max altitude to sea bottom and mountain top
  z_min <- terra::global(topo_init, "min")
  z_max <- terra::global(topo_init, "max")
  
  topo_calc <- terra::ifel(
    topo_init > 0, 
    topo_init * .alt / as.numeric(z_max), 
    topo_init * (.alt / 4) / abs(as.numeric(z_min))
  )
  ...
  topo <- terra::focal(topo_calc, w = matrix(1/31^2, nc=31, nr=31), pad = T, padValue = 0) ## Supposed to be faster
  ...
  ## Get forest types from a midpoint displacement neutral landscape model (MPD).
  if (!is.null(.seed)) set.seed(.seed)
  ft_mpd <- NLMR::nlm_mpd(nrow = mp+5,  ncol = mp+5, resolution = rr, roughness = 0.7, rescale = T, verbose = F)
  ft_mpd <- terra::rast(ft_mpd)
  ft_mpd <- terra::crop(ft_mpd, terra::ext(0, ll, 0, ll))
  ft_mpd <- terra::aggregate(ft_mpd, fact = 10, fun=mean) 
  
  ## Add clustering
  ft_param2 <- c(0.4, 0.2, 0.1, 0.2, 0.1)
  test <-  c(0.4, 0.2, 0.1, 0.2, 0.1)
  ft_ne <- nlm_neigh(
    nrow = mp/10,  
    ncol = mp/10, 
    resolution = rr*10, 
  ...
  #   time1 <- Sys.time()
  #   
  #   mg <- terra::ifel(topo >= -10 & topo <= 20, 6, NA)
  #   # mg[mg <  -10 | mg >  20] <- NA
  #   # mg[mg >= -10 & mg <= 20] <- 1
  #   # mg[,1:600]    <- NA
  #   # mg[700:1000,] <- NA
  ...
  ## --- Sea mask -------------------------------------------------------------
  sea_mask <- terra::ifel(topo < 0, 0, NA)
  # plot(sea_mask)
  # plot(mg)
  
  ## Added land covers based on masks
  lc <- terra::ifel(topo < 0, 0, ft)
  if (.mg) lc <- terra::ifel(topo >= -10 & topo <= 20, 6, lc)
  ...
  # lc
  # plot(lc, col = lc_param$hex, axes = F)
  ...
  topo_mat <- terra::as.matrix(flip(topo), wide = TRUE)
  ...
  out_gr <- plotly::plot_ly(
    ...
    plotly::add_surface(
      ...
      #showscale=F
      surfacecolor = as.matrix(flip(lc), wide = T)
    ) %>%
    # plotly::add_trace(data = NULL, type = "mesh3d", 
    #           x = water$x,
    #           y = water$y,
    #           z = water$z,
    ...
    plotly::layout(scene = list(aspectmode="manual", aspectratio = list(x=1, y=1, z=0.3))) ## 0.1
  out_gr
